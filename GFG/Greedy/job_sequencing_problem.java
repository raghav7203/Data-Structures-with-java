...
class struct{
    int i, d, p;
    struct(int i, int d, int p){
        this.i=i;
        this.d=d;
        this.p=p;
    }
}
class sortComparator implements Comparator<struct>{
	...
	public static void main (String[] args)throws IOException {
        ...
        for(int t=1;t<=test;t++){
            ...
            for(int i=0;i<n*3;i=i+3){
                d[c1++]=Integer.parseInt(s[i+1]);
                p[c2++]=Integer.parseInt(s[i+2]);
            }
            
            ArrayList<struct> obj=new ArrayList<>();
            for(int i=0;i<n;i++){
                obj.add(new struct(i, d[i], p[i]));
            }
            ...
            int a[]=new int[n];
            // for(int i=0;i<n;i++)
            //     System.out.println(obj.get(i).i+" "+obj.get(i).d+" "+obj.get(i).p);
            
            for(int i=0;i<n;i++){
                for(int j=Math.min(obj.get(i).d-1, n-1);j>=0;j--){
                    if(a[j]==0){
                        a[j]=1;
                        c++;
                        profit+=obj.get(i).p;
