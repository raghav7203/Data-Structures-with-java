...
Output:
For each testcase, there will be three lines. First-line contains inorder traversal of the tree(after extracting leaves). The second and third line contains the nodes of DLL, first in order of inorder traversal of tree and next in reverse order.

User Task:
The task is to complete the function convertToDLL() which takes the root of the given binary tree as input and returns the head of the doubly link list. This function should extract the leaf nodes into a doubly link list and modify the original binary tree by excluding the leaf nodes. 
...
class Tree{
    ...
    Node leaves(Node root, ArrayList<Integer> a){
        if(root==null)
            return null;
            ...
            root.left=leaves(root.left, a);
            return root;
        }
        else if(root.left==null && root.right!=null){
            root.right=leaves(root.right, a);
            return root;
        }
        else{
           root.left=leaves(root.left, a);
